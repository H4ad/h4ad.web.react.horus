...
const buildCommand = process.env.REACT_APP_BUILD_COMMAND;
const buildFolder = 'build/';

/*Build your web app -- Command to build web app*/
const buildWebAppCommand = `yarn run ${ buildCommand }`;
...
const s3AddHeadersCommand = `aws --profile ${ profile } s3 cp s3://${ bucketName } s3://${ bucketName } --recursive --exclude \"*\" --include \"*.html\" --include \"manifest.json\" --metadata-directive REPLACE --acl public-read --cache-control max-age=0,must-revalidate,public --content-type \"text/html; charset=utf-8\"`;
